...
public class AuthService {
 ...
 public Registration register(RegisterRequest req){
   ...
   User user=new User(); user.setFullName(req.fullName().trim()); user.setEmail(email); user.setPasswordHash(encoder.encode(req.password())); user.setRole(Role.PARTICIPANT); user.setEnabled(false); users.save(user);
   Instant consentTime=Instant.now(); Registration reg=new Registration(); reg.setUser(user); reg.setRegistrationCode(code(req.participationType())); reg.setDateOfBirth(req.dateOfBirth()); reg.setPhone(req.phone().trim()); reg.setCountry(req.country().trim()); reg.setCity(req.city().trim()); reg.setParticipationType(req.participationType()); reg.setDomain(req.domain().trim()); reg.setAgeVerificationStatus("DOB_VERIFIED"); reg.setGuardianConsentRequired(age<18); reg.setGuardianConsentReceived(age>=18); reg.setStatus(age<18?RegistrationStatus.PENDING_GUARDIAN:RegistrationStatus.PENDING_SUBMISSION); reg.setGuardianName(age<18?req.guardianName().trim():null); reg.setGuardianEmail(age<18?req.guardianEmail().trim().toLowerCase(Locale.ROOT):null); reg.setGuardianRelationship(age<18?req.guardianRelationship().trim():null); reg.setGuardianPhone(age<18?req.guardianPhone().trim():null); reg.setGuardianCountry(age<18?req.guardianCountry().trim():null); reg.setTermsAcceptedAt(consentTime); reg.setPrivacyAcceptedAt(consentTime); reg.setRulesAcceptedAt(consentTime); reg.setMarketingConsent(req.marketingConsent()); registrations.save(reg);
